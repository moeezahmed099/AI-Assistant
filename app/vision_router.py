            ...
            if check_pipeline_run_exists(clean_run_id):
                ...
                if file is not None:
                    try:
                        ...
                        logger.warning(f"Could not record query asset for run {clean_run_id}: {e}")
                ...
                logger.info(f"Pipeline run {clean_run_id} transitioned to vision_complete via search endpoint.")
        except Exception as exc:
            logger.warning(f"Failed to record search results for pipeline run {selected_run_id}: {exc}")
